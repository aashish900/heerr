...
**Status (2026-07-11):** Phases A–O complete. Phase O (song metadata edit, issue #44) shipped 2026-07-06 at `4.6.2`; `4.7.0` (2026-07-10) is the app-wide gradient redesign; `4.7.1`/`4.7.2`/`4.7.3`/`4.7.4` (2026-07-10–11) are widget/tab visual polish passes; `4.8.0` (2026-07-11) is the Android Home Screen redesign; `4.8.1` (2026-07-11) is a redesign fix pass (Android-side only — no backend changes; version bumped for sync); `4.8.2` (2026-07-11) is a second redesign fix pass (Android-side only — no backend changes; version bumped for sync); `4.8.3` (2026-07-11) is a third redesign fix pass (Android-side only — no backend changes; version bumped for sync); `4.8.4` (2026-07-11) is a fourth redesign fix pass (Android-side only — no backend changes; version bumped for sync); `4.8.5` (2026-07-11) is a fifth redesign fix pass and merges `redesign/home-screen` into `main` (Android-side only — no backend changes; version bumped for sync); `4.9.0` (2026-07-11) is the Android Profile screen redesign (Android-side only — no backend changes; version bumped for sync); `4.10.0` (2026-07-11) is the Android Library screen redesign (Android-side only — no backend changes; version bumped for sync); `4.11.0` (2026-07-11) is the Android Now Playing screen redesign (Android-side only — no backend changes; version bumped for sync); `4.11.1` (2026-07-12) is a Now Playing + Profile fix pass (Android-side only — no backend changes; version bumped for sync); `4.11.2` (2026-07-12) fixes the Library Favorites tile to show its song count (Android-side only — no backend changes; version bumped for sync); `4.12.0` (2026-07-12) is the Android Downloads screen redesign ("Sync Center", Phase DL) (Android-side only — no backend changes; version bumped for sync); `4.12.1` (2026-07-12) is a Downloads hero fix pass — green Online status pill + real server illustration (Android-side only — no backend changes; version bumped for sync); `4.13.0` (2026-07-12) is the Android Settings screen redesign ("Control Center", Phase SE) (Android-side only — no backend changes; version bumped for sync). `4.14.0` (2026-07-13) adds server-resolved `cover_url` to `POST /recommend` results (derived from the watch URL's video id) so the Android client no longer parses source URLs on-device. `4.14.1` (2026-07-13) fixes a strict-mypy list-item error in `cover_url_for_source_url` (type-only; no behavior change). `4.14.2` (2026-07-13) is a CI-only release (no backend changes; version bumped for sync). `4.14.3` (2026-07-13) fixes an Android ANR — a reentrancy guard on the audio handler's play path (no backend changes; version bumped for sync). `4.14.4` (2026-07-13) is an Android Downloads hero fix pass — full-bleed server image + a live sync-progress bar/count fix (Android-side only — no backend changes; version bumped for sync). `4.14.5` (2026-07-13) is a second Android Downloads hero fix pass (corner clipping + edge fade) plus a "Clear all downloads" reliability fix (Android-side only — no backend changes; version bumped for sync). `4.14.6` (2026-07-13) fixes an Android black-screen-on-widget-launch bug — deferred `Workmanager().initialize()` off the `runApp()` blocking path + a branded launch splash (Android-side only — no backend changes; version bumped for sync). `4.14.7` (2026-07-13) is a third Android Downloads hero fix pass — server illustration width +5% and a new higher-resolution illustration asset (Android-side only — no backend changes; version bumped for sync). `4.14.8` (2026-07-14) is a home-screen widget transport/divider resize pass (Android-side only — no backend changes; version bumped for sync). `4.14.9` (2026-07-14) is a widget divider fix-up — tapers and fades on both ends instead of just the bottom (Android-side only — no backend changes; version bumped for sync). `5.0.0` (2026-07-20) is Phase P — podcasts (#53): own data model (channels/episodes/subscriptions/progress), Podcast Index + RSS discovery, episode downloads via a plain HTTP fetch (no spotDL/yt-dlp involved), and Range-capable episode audio serving + resume-position sync — Navidrome does not support podcasts server-side, so the backend now owns podcast storage and playback end to end. `5.0.1` (2026-07-20) swaps podcast discovery from Podcast Index to Apple's iTunes Search API (no signup/key needed) after Podcast Index's signup form began rejecting free-email-provider addresses — the client-visible search contract is unchanged, so no Android changes were needed. `5.0.2` (2026-07-20) is Android-side only — a "subscribe by feed URL" affordance on the Discover screen, using the already-existing `POST /podcasts/subscribe` raw-`feed_url` path (no backend changes; version bumped for sync). `5.1.0` (2026-07-20) is Android-side only — Phase PR1 (#53), podcasts move into Library as a first-class Music/Podcasts content switch with a richer show-detail screen and episode rows (no backend changes; version bumped for sync). `5.2.0` (2026-07-20) is Android-side only — Phase PR2 (#53), the podcast player redesign (skip±30s transport, working playback-speed control, plain scrubber, no backend changes; version bumped for sync). `5.3.0` (2026-07-20) is Phase PA — podcast aggregate feeds (#53): `GET /podcasts/episodes?filter=in_progress|latest|downloaded` returns episodes across every show a user is subscribed to (with channel title/art inlined), backing the Android redesign's Home "Continue Listening"/"Latest Episodes" sections and the Library Episodes/Downloads tabs; the per-channel `GET /podcasts/channels/{id}/episodes` gained a `sort=newest|oldest|unplayed` param for the Episodes-list sort control. Android Phase PR3 (#53) shipped against this same `5.3.0` — see `android/docs/ROADMAP.md` — closing out the podcast-flow redesign (backend-side changes: none beyond PA1/PA2 above). `5.3.1` (2026-07-20) is Android-side only — Home's Podcasts empty state was missing the "Discover podcasts" button (no backend changes; version bumped for sync). `5.3.2` (2026-07-20) adds `episode_id` to `JobView` (`GET /queue`, `GET /status/{id}`) — always null for song/album/playlist jobs, set for episode jobs — so the Android Queue screen's Retry action can tell episode jobs apart from song jobs and re-dispatch each through the right endpoint (`POST /podcasts/episodes/{episode_id}/download` vs `POST /download`); fixes a user-reported bug where retrying a failed episode download 422'd (the enclosure URL isn't a YouTube/YouTube Music URL, which `POST /download` requires). `5.3.3` (2026-07-20) is Android-side only — fixes a CI-flaky test (`NowPlayingPersistence.dispose()` racing an in-flight write against test teardown), unrelated to the podcast work (no backend changes; version bumped for sync). `5.3.4` (2026-07-21) is a user-reported infra fix — episode downloads failed with "Permission denied" writing to `/data/media/podcasts`: the backend container runs as non-root UID 1000, but (unlike `/data/postgres`, which has always had a `heerr-postgres-init` one-shot chown) nothing on the host ever chowned this newer bind-mount path, so Docker's root-owned auto-created directory blocked every write. `docker-compose.snippet.yml` gains a `heerr-podcasts-init` service mirroring the Postgres pattern; `heerr-backend` now depends on it completing first. `5.3.5` (2026-07-21) is Android-side only — fixes a second, previously-missed copy of the Queue Retry bug (#53) in `job_detail_screen.dart`'s own separate `_retry` implementation (no backend changes; version bumped for sync).
...
### [x] P6. Episode audio serving (Range) + progress + version bump
...
## Phase PA — Podcast aggregate feeds (#53)

Backend half of the Android podcast-flow redesign (`android/docs/ROADMAP.md` Phase PR3; plan file `~/.claude-personal/plans/luminous-singing-pinwheel.md`). Phase P lists episodes **one channel at a time only**; the redesign's Home sections ("Continue Listening", "Latest Episodes") and the Library "Episodes"/"Downloads" tabs need **cross-subscription** feeds, plus a sort option on the per-channel list. Backend-first: Android PR3 must not start until PA is deployed and curl-testable. No new tables — pure query work over the Phase P schema.

**Order of execution: PA1 → PA2.** Suggested version bump `v5.3.0` (shared with Android PR3 — version-sync rule, `/CLAUDE.md` §3; owner to confirm).

### [x] PA1. Cross-subscription episode feed — `GET /api/v1/podcasts/episodes`
**Files:** `backend/app/api/v1/podcasts.py` (+route), `backend/app/schemas/podcast.py` (new `EpisodeWithChannelItem` carrying channel title + art alongside each episode), optional query helper under `backend/app/services/`, `backend/tests/test_podcast_feeds.py`.
**Deliverable:** `GET /podcasts/episodes?filter=in_progress|latest|downloaded&limit=&offset=` (`require_scope("read")`), aggregating episodes across the **caller's** subscriptions (join `podcast_subscription` → channel → episode, left-join this user's `podcast_progress`):
- `in_progress`: `position_s > 0 AND NOT played`, order `last_played_at DESC` → Continue Listening.
- `latest`: order `published_at DESC NULLS LAST` → Latest Episodes.
- `downloaded`: `downloaded_path IS NOT NULL`, order `downloaded_at DESC` → Downloads tab.
Each item carries channel `id`/`title`/`image_url` so the client can render the show without a second call.
**Test gate:** 401/403; two-user isolation (user-A's feed excludes user-B's subscriptions/progress); each filter returns the right set + ordering; pagination bounds; empty result → `[]`. Full suite green; ruff + mypy clean.
**Commit:** `feat(backend): PA1 — cross-subscription episode feed GET /podcasts/episodes (#53)`

### [x] PA2. Per-channel episode sort + version bump
**Files:** `backend/app/api/v1/podcasts.py` (`list_episodes` gains `sort`), `backend/tests/test_podcast_episodes.py` (extend), docs (`DECISIONLOG.md` ADR, `PODCASTS.md` new endpoints, `CHANGELOG.md`), version files per `/CLAUDE.md` §3.
**Deliverable:** Add `sort=newest|oldest|unplayed` (default `newest`, preserving current behavior) to `GET /podcasts/channels/{id}/episodes` to back the redesign's Episodes-list sort control — `oldest` = `published_at ASC NULLS LAST`; `unplayed` orders un-played episodes first then newest. ADR records why an aggregate feed (vs. per-channel only) and the sort semantics.
**Test gate:** each `sort` value returns the documented ordering; default unchanged; 422 on a bad value. Full suite green; ruff + mypy clean.
**Smoke (home server):** curl each `filter` + each `sort` with a real read token → expected episodes + channel fields; 401/403 on bad scope.
**Commit:** `feat(backend): PA2 — per-channel episode sort + v5.3.0 (#53)`
