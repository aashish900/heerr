...
## 2026-07-12 — Settings redesign SE6: Profiles + Recommendations flattened, _CollapsibleSection retired

- `lib/screens/settings/settings_screen.dart` — Profiles and Recommendations are no longer wrapped in `_CollapsibleSection`/`ExpansionTile`; each is now a plain `SettingsSectionHeader` + `SettingsGroupCard(children: [ProfilesSection()])` / `SettingsGroupCard(children: [_RecommendationsSection()])`. **Deleted the `_CollapsibleSection` class** — no callers remain after this task (D5 flattening, started in SE5, now complete). Inner row logic (profile switch/remove dialogs, engine-health chips) is untouched — only the outer wrapper changed, per the plan's "zero logic change" scope for this task.
- `lib/screens/settings/settings_tiles.dart` — `SettingsGroupCard` now wraps its `Column` in `Material(type: MaterialType.transparency)`. Needed because `ProfilesSection`/`_RecommendationsSection` contain plain `ListTile`s (not pre-wrapped in their own `Material`, unlike `SettingsTile`); without an explicit transparent Material between the card's decorated `Container` and those tiles, Flutter's debug assertion flags the background color as liable to hide ink/selection effects.
- `test/screens/settings_screen_test.dart` — the "Collapsible sections (#17)" group is replaced with "Flat sections (#17 retired D5, SE6)": one test asserting every section header renders and every body (Profiles' "Add profile", Offline's "WiFi only", Recommendations' "Engine health") is visible without tapping. All four `_expandSection(tester, 'Recommendations')` calls removed along with the now-unused `_expandSection` helper.
- Verification: `flutter analyze` clean; full `flutter test` green (983 tests).

## 2026-07-12 — Settings redesign SE7: About footer + version bump to 4.13.0

- New `lib/screens/settings/about_footer.dart`: `AboutFooter` — app version row (reuses `appVersionProvider`, key `settings-app-version` preserved from the retired `_AppVersionTile`), "Open source licenses" row (`showLicensePage`), "GitHub" row (opens `https://github.com/aashish900/heerr` via `url_launcher`, injectable `onGithubTap` for tests), and a "Made for self-hosted music lovers" tagline. No Privacy Policy / Terms rows (no such documents exist — D1: don't ship dead links).
...
- `lib/screens/settings/settings_screen.dart` — replaced `_AppVersionTile` with `AboutFooter()` at the bottom of the scroll; dropped the now-unused `appVersionProvider` import (moved into `about_footer.dart`).
...
## 2026-07-13 — fix: Downloads hero left corners + edge fade (follow-up to v4.14.4)

- User feedback against the source mockup after v4.14.4 shipped: the hero card's left-side (top/bottom) rounded corners weren't visible on a real Pixel 7, and the image had a hard seam where it met the text panel instead of fading in.
- Root cause, confirmed by comparing a `flutter test` golden render (Skia) against the on-device Impeller/OpenGLES render: `Container.clipBehavior`'s implicit clip (derived from `Decoration.getClipPath`) rounded the corners correctly in the Skia golden but reportedly missed on-device — Impeller has known corner-rounding discrepancies for this kind of bleed-to-edge-image `ClipPath`.
- **`lib/screens/downloads/server_status_card.dart`** — wrapped `ServerGlyph` in an explicit `ClipRRect` (`topLeft`/`bottomLeft: Radius.circular(24)`, extracted to a shared `_kHeroRadius` constant) instead of relying solely on the ambient `Container` clip. Added a `ShaderMask` (`BlendMode.dstIn`, left-to-right gradient fading to transparent over the last 40% of the image width) around the same image, mirroring the Home hero card's existing art-edge fade technique (`continue_listening_card.dart`) so the image blends into the panel instead of a hard cut.
- Verified via a throwaway golden-image capture (`test/screens/downloads/_hero_visual_debug_test.dart`, deleted after use) rather than a live device — corners render consistently rounded on all sides and the fade is visible; a live Pixel 7 re-check is still pending since it requires real Navidrome credentials to get past login.
- `flutter analyze` clean; full `flutter test` green (984 tests, no test count change — no new tests added, existing suite unaffected).

## 2026-07-13 — fix: "Clear all downloads" needed multiple taps ("Directory not empty")

- User-reported bug (Settings → Downloads & Storage → Clear all downloads): tapping it usually failed with a "Directory not empty" error, but retrying manually a few times eventually cleared everything.
- Root cause: `serverRoot` (`lib/offline/offline_paths.dart:48`) holds `songs/`, `covers_hi/`, `lyrics/`, and `library_cache/` — subdirectories written independently by several unrelated code paths (the offline sync downloader, cover-art caching, lyrics caching, library-list caching), all of which can fire while the user is simply browsing the app. `Directory.delete(recursive: true)` snapshots the tree once, then deletes bottom-up; any of those writers dropping a file into a subdirectory mid-walk makes that subdirectory's delete fail with `ENOTEMPTY` (or occasionally `ENOENT` if an entry vanished between listing and deleting it) — a classic TOCTOU race, not a permissions or logic bug. Confirmed by writing a plain-Dart (non-widget) test with a real concurrent `Timer.periodic` writer racing a real `Directory.delete` — reproduced the exact failure.
- **`lib/offline/delete_recursive_with_retry.dart`** (new) — extracted `deleteRecursiveWithRetry(Directory, {attempts = 5})`: retries the delete on any `FileSystemException`, re-snapshotting the tree each time so a transient extra/missing entry gets swept up on the next pass. This automates exactly what manually re-tapping the button already did.
- **`lib/screens/settings/settings_offline.dart`** — `_ClearAllTileState._confirm()` now pauses `OfflineSync` (via its existing `pause()`/`resume()`, already used by `LifecycleCoordinator` for background/foreground transitions) before deleting, closing off the sync downloader as one of the concurrent-writer sources, and calls the new retry helper instead of a single-shot delete. Also fixed a latent bug found while testing this: the `finally` block called `ref.read(offlineSyncProvider.notifier).resume()` unconditionally, which throws "Cannot use ref after the widget was disposed" if the user navigates away from Settings while the clear is still in flight — now guarded by the same `mounted` check as the adjacent `setState`.
- **`test/offline/delete_recursive_with_retry_test.dart`** (new, 4 tests) — clean-delete, already-gone no-op, the concurrent-writer race (the actual regression), and a permanent-failure case (chmod 555'd directory) proving the retry loop still surfaces a genuine non-transient failure instead of swallowing it forever. Plain `test()`, not `testWidgets()` — deliberately avoids Flutter's `FakeAsync` test-clock, which doesn't let a real concurrent `Timer`/`Future.delayed` race a real `Directory.delete` inside a widget test (confirmed the hard way: an earlier widget-test attempt at this same regression deadlocked because `Directory.delete`'s completion and the retry's `Future.delayed` both got stuck behind the fake clock).
- `flutter analyze` clean; full `flutter test` green (988 tests, 4 new).

## 2026-07-13 — fix: black screen opening the app from the home-screen widget

- User-reported bug: tapping the hero home-screen widget often opened the app to a plain black screen (no spinner, no logo), even with network/WiFi/Tailscale confirmed up. Not a network bug — the network calls all live inside the Flutter UI, which hadn't rendered a single frame yet.
- Root cause: `lib/main.dart`'s `main()` runs `Workmanager().initialize(...)` then `await AudioService.init(...)` (binds to a native `MediaBrowserService` / starts the playback foreground service) then two local credential migrations — all `await`ed **before** `runApp()` is ever called. Android can't remove the launch splash until Flutter draws its first frame, so any delay in that native-init chain is a delay before anything paints. `AudioService.init()`'s foreground-service start is exactly the kind of call Android's background-activity-start throttling can stall longer when the launch comes from a widget tap on an idle/doze'd device, vs. an interactive launcher tap — matching the "often" in the report.
- Compounding factor: `drawable-v21/launch_background.xml` used `?android:colorBackground`, which resolves to solid black on this dark-only app's `LaunchTheme` (`Theme.Black.NoTitleBar`) — the stock, never-customized Flutter template splash. So the entire native-init window rendered as flat black with zero loading feedback, indistinguishable from a hang.
- **`android/app/src/main/res/drawable/launch_background.xml`, `drawable-v21/launch_background.xml`** — both now show the heerr gradient logo (`@drawable/ic_launcher_foreground`, already shipped for the adaptive launcher icon) centered on black instead of a flat colour, so the native-init window reads as "loading heerr" rather than "broken."
...
## 2026-07-14 — fix: home-screen widget transport sizing + gradient divider

- User request: enlarge the Previous/Next transport buttons 20% and the Play/Pause button 30% in the hero home-screen widget, and restyle the flat divider next to the heerr logo to use the brand gradient with a tapered, fading-out look.
- **`android/app/android/app/src/main/res/layout/hero_widget.xml`** — both idle and playing transport rows resized (button dimension and internal padding scaled together so the icon's drawn area grows proportionally): idle/playing Previous/Next `40dp`/`36dp` → `48dp`/`44dp` (padding `8dp`/`7dp` → `10dp`/`9dp`); idle/playing Play `44dp` → `57dp` (padding `8dp`/`11dp` → `10dp`/`14dp`). The `#2E2E2E` flat-color `LinearLayout` divider next to the logo is replaced with an `ImageView` (`2dp` × `52dp`, was `1dp` × `40dp` — widened so the taper reads, lengthened 30%) pointing at a new drawable.
...
## 2026-07-20 — feat: PC2 — podcast discover + subscribe (#53)

- **`android/app/lib/providers/podcasts/podcast_search.dart`** (new) — `podcastSearchProvider` family, debounced via the existing `searchDebounceProvider` (reused from `providers/search.dart` rather than duplicating the constant); empty/whitespace queries short-circuit without a network call; in-flight requests are cancelled on query change via a `CancelToken` (same pattern as `ytmSearch`). `BackendService.searchPodcasts` gained a `cancelToken` param to support this.
- **`android/app/lib/providers/podcasts/podcast_subscriptions.dart`** (new) — `PodcastSubscriptionsProvider` (keep-alive `AsyncNotifier`) wrapping `GET /podcasts/subscriptions`, with `subscribe(feedUrl)`/`unsubscribe(channelId)` methods that mutate local state after the backend call succeeds. Also exports `isFeedSubscribed`/`subscribedChannelFor` pure helpers (matched by `feedUrl`, since a fresh search result has no `id`). Deliberately the single source of subscription truth — PC3's Subscriptions screen will read the same provider instead of a duplicate list.
- **`android/app/lib/screens/podcasts/discover_screen.dart`** (new) — search field → result list (art/title/author, check-mark when already subscribed) → tap opens a channel-preview bottom sheet with a Subscribe/Unsubscribe `FilledButton` that reflects live subscription state. Loading/empty/error states follow the existing `SkeletonList`/`EmptyState`/`showApiError` conventions (`library_search_results.dart`, `recommendations_screen.dart`) — no bespoke "podcast search error" copy needed since the existing `HttpStatusError` snackbar already surfaces the backend's 502 detail message.
- **`android/app/lib/router.dart`** — added `Routes.podcastsDiscover` (`/podcasts/discover`) as a top-level push route (same shape as `/player`). No bottom-nav / Profile entry point yet — PC3 wires that in alongside the Subscriptions screen.
- Tests: `test/screens/podcasts/discover_screen_test.dart` (7 widget tests — empty/results/error/subscribed-badge/subscribe-toggle/unsubscribe-toggle), `test/providers/podcasts/podcast_search_test.dart` (2), `test/providers/podcasts/podcast_subscriptions_test.dart` (4).
- `flutter analyze` and `flutter test` (1018 tests) green before and after. No on-device smoke — no `adb` device connected this session; PC2's documented test gate is widget tests only (manual smoke is PC5's gate).

## 2026-07-20 — feat: PC3 — subscriptions + channel episode list (#53)

- **`android/app/lib/providers/podcasts/podcast_episodes.dart`** (new) — `PodcastEpisodesNotifier` family (keyed by `channelId`), page size 20. `build()` loads page 1; `loadMore()` appends the next page at the right offset (no-ops once `PodcastEpisodePage.hasMore` is false); `refresh()` calls `POST /podcasts/channels/{id}/refresh` (re-pulls the RSS feed server-side) before reloading page 1 — so pull-to-refresh actually surfaces newly-published episodes, not just a re-read of cached rows.
- **`android/app/lib/providers/podcasts/podcast_subscriptions.dart`** — added a `refresh()` method (same `invalidateSelf()` + `await future` shape as `Recommendations.refresh()`) for the Subscriptions screen's `RefreshIndicator`; the notifier's `future` getter is `@protected`, so the screen can't call it directly.
- **`android/app/lib/screens/podcasts/subscriptions_screen.dart`** (new) — 2-column grid of subscribed channels (art/title/author), an AppBar "add" action pushing Discover, pull-to-refresh, and an empty state with a "Discover podcasts" CTA when nothing is subscribed yet.
- **`android/app/lib/screens/podcasts/channel_screen.dart`** (new) — paginated episode list (scroll-to-bottom triggers `loadMore()`), each row showing published date + duration, or "Resume at MM:SS" for a partially-played episode, or a check-mark for a played one. Pull-to-refresh wired to the notifier's `refresh()`. Download (PC4) and playback (PC5) actions are intentionally absent this milestone — rows are read-only.
- **`android/app/lib/router.dart`** — added `Routes.podcastsSubscriptions` (`/podcasts/subscriptions`) and `/podcasts/channel/:id`, both top-level pushes (same shape as `/podcasts/discover`).
- **`android/app/lib/screens/profile/profile_screen.dart`** — added a "Podcasts" row to the "My Music" section (`context.push(Routes.podcastsSubscriptions)`), the nav entry point promised at PC2. `push`, not `go`, since podcasts routes are top-level rather than `ShellRoute`-nested — no duplicated-page-key risk.
- Fixed 3 pre-existing `profile_screen_test.dart` "Log Out" tests that broke because the new Podcasts row pushed the Log Out button further down the scroll view than a single `scrollUntilVisible` cleared past the pinned AppBar — applied the same "extra `-80` drag" fix already used by the adjacent Settings-card test.
- Tests: `test/providers/podcasts/podcast_episodes_test.dart` (4), `test/screens/podcasts/subscriptions_screen_test.dart` (4), `test/screens/podcasts/channel_screen_test.dart` (6), plus one new `profile_screen_test.dart` case for the Podcasts row.
- `flutter analyze` and `flutter test` (1033 tests) green before and after. No on-device smoke — no `adb` device connected this session.

## 2026-07-20 — feat: PC4 — podcast episode download via Sync Center (#53)

- **Critical fix, `android/app/lib/models/enums.dart`** — `ContentType` had no `episode` `JsonValue`, so `JobView.fromJson` (used by `GET /queue` and `GET /status/{id}`) would throw on any episode job the moment PC4 started dispatching them. Same class of bug as the backend's P5 `JobView.source_type` Literal fix. Added `@JsonValue('episode') episode` and a regression test (`test/models/job_view_test.dart`) round-tripping a `source_type: "episode"` job.
- **`android/app/lib/providers/podcasts/podcast_episode_download.dart`** (new) — `PodcastEpisodeDownloadDispatcher`, same in-flight-set shape as `providers/download.dart::DownloadDispatcher`, calling `POST /podcasts/episodes/{id}/download`. Dispatched jobs reuse the existing `jobs` queue (`source_type == 'episode'`), so they already surface in the existing Queue screen (`GET /queue`) with no further wiring needed there.
- **`android/app/lib/screens/podcasts/channel_screen.dart`** — `_EpisodeTile` is now a `ConsumerWidget` with a trailing Download `IconButton` (spinner while in-flight, static "Downloaded" badge once `episode.downloaded` — the row doesn't poll; that flips true on the channel's next pull-to-refresh). Tapping Download shows a "Queued: <title>" snackbar on success, `showApiError` on failure — same affordance as `library_search_results.dart::_downloadOnly` for song downloads.
- **Scope note**: episodes are deliberately **not** added to the Downloads screen's Songs/Albums/Playlists tabs (`lib/screens/downloads/*`) or the "Sync Center" activity cards — those are driven by the on-device offline-manifest system (`providers/sync_activity.dart`) for songs already in the Navidrome library, which episodes (stored in `PODCAST_OUTPUT_DIR`, never Navidrome-indexed) structurally aren't part of. Episode download progress is visible via the existing generic Queue screen instead, which already renders any `JobView` regardless of `sourceType`. Retry/Play actions on the Queue screen remain song-oriented (`playJobDoneFromSubsonic` no-ops gracefully with a "can't search" snackbar for a done episode job; Retry would need `episode_id` added to the backend's `JobView` schema, which is out of scope for an Android-only milestone) — flagging as a known gap, not silently glossed over.
- Tests: `test/models/job_view_test.dart` (1), `test/providers/podcasts/podcast_episode_download_test.dart` (1), 4 new cases in `test/screens/podcasts/channel_screen_test.dart` (download button visible/hidden, dispatch + Queued snackbar, ApiError snackbar on failure).
- `flutter analyze` and `flutter test` (1039 tests) green before and after. No on-device smoke — no `adb` device connected this session; PC4's test gate is widget/unit tests (manual smoke is PC5's gate).

## 2026-07-20 — feat: PC5 — podcast player + progress sync + v5.0.0 (#53)

Closes out Phase PC (podcasts, #53). See `DECISIONLOG.md` 2026-07-20 "PC5: podcast episode playback + progress sync" for the full rationale behind the deviations from the original `ROADMAP.md` PC5 wording — summarized here.

- **`android/app/lib/player/episode_to_media_item.dart`** (new) — `episodeToMediaItem`, the fourth `MediaItem.id` flavour (alongside file:// / Subsonic-stream / preview-proxy). **Never** produces a `file://` id — there is no on-device episode-download feature, unlike songs — so a downloaded (server-side) episode plays via the Range-capable `GET /podcasts/episodes/{id}/audio?token=` proxy, and a not-yet-downloaded one streams `enclosureUrl` directly. `extras['episodeId']`/`extras['channelId']` mark the item; `isEpisodeMediaItem`/`episodeIdFromMediaItem` mirror `isPreviewMediaItem`.
- **`android/app/lib/player/episode_progress_controller.dart`** (new) — `EpisodeProgressController`, same stream-driven shape as `scrobble_controller.dart::ScrobbleController` (no `Timer.periodic`): throttles `positionStream` ticks to one report per `minInterval` (15s default), force-reports immediately on a `playbackState.playing` pause/stop transition, and force-reports the outgoing episode's last position on a track change. Always reports `played: false` in this milestone (completion detection deferred).
- **`android/app/lib/player/episode_progress_provider.dart`** (new) — `episodeProgressProvider`, keep-alive wiring to the real `HeerrAudioHandler` + `BackendService.updateEpisodeProgress`, watched from `HeerrApp.build` (`main.dart`) alongside `scrobbleProvider`/`nowPlayingPersistenceProvider` for its side effect.
- **`android/app/lib/player/podcast_playback_actions.dart`** (new) — `playEpisode`: replaces the queue with the tapped episode (same "replace, don't append" shape as `playSongFromSubsonic`/`playPreview`), then seeks to `episode.positionS` when non-zero — the resume-from-position-on-open half of PC5.
- **`android/app/lib/screens/podcasts/channel_screen.dart`** — episode rows are now tap-to-play (`onTap: () => playEpisode(...)`), alongside PC4's Download action.
- **Known follow-up, not silently omitted:** the Now Playing / Mini Player screens were not retrofitted with episode-specific chrome (no skip-15s, playback speed, show-name treatment, or "Podcast" badge parallel to the Preview badge) — playback itself works generically, but the richer UI is deferred.
- Version: already at `v5.0.0` across all 5 sync locations (bumped at backend P6, ahead of this Android milestone) — no further bump needed; this entry documents Phase PC's completion against that version.
...
## 2026-07-20 — feat: subscribe by feed URL on the Discover screen (#53)

- **User request:** `POST /podcasts/subscribe` has always accepted a raw `feed_url` — nothing backend-side gates a subscribe to only feeds that came back from search. This exposes that path from the UI, both as a fallback for shows the discovery source doesn't index and as a direct route for a user who already has the feed URL.
- **`android/app/lib/screens/podcasts/discover_screen.dart`** — new `_SubscribeByUrlBar` (a `TextField` + icon button) rendered below the search field, always visible. Validates the input is a parseable `http`/`https` URL before dispatching; on success, clears the field and shows a "Subscribed: `<title>`" snackbar; on failure, routes through the existing `showApiError` convention. Calls `podcastSubscriptionsProvider.notifier.subscribe(feedUrl)` — the same method PC2's preview-sheet Subscribe button already uses, so subscriptions from either path land in the same state and the same list on the Subscriptions screen.
