...
## 2026-07-20 — PC5: podcast episode playback + progress sync (#53)

**Context:** PC5 closes out Phase PC (podcasts) — a fourth `MediaItem.id` flavour for episodes, alongside the existing file:// / Subsonic-stream / preview-proxy kinds, plus throttled resume-position sync. `android/docs/ROADMAP.md`'s original PC5 wording (written speculatively at planning time, before P1–P6 shipped) said episode playback should use `file://` "when downloaded." That's wrong for this app: unlike songs, there is no on-device episode-download feature — `PodcastEpisode.downloaded` means the **backend** has the bytes in `PODCAST_OUTPUT_DIR` (Phase P), not the phone. There is no local file path anywhere in the client's data model to build a `file://` URI from.

**Decision:**
1. **`episodeToMediaItem` never produces a `file://` id.** Downloaded (server-side) episodes play via the Range-capable `GET /podcasts/episodes/{id}/audio?token=` proxy (real seek/resume); not-yet-downloaded episodes stream from `PodcastEpisode.enclosureUrl` directly — never proxied, matching the backend's own `backend/docs/PODCASTS.md` §2.5 design decision.
2. **Progress reporting (`EpisodeProgressController`) mirrors `ScrobbleController`'s shape exactly** — subscribes to `mediaItem.stream` + `player.positionStream` + `playbackState.stream`, no `Timer.periodic`. The ~15s cadence falls out of a wall-clock throttle on the position-stream ticks (which `just_audio` already emits regularly during playback); `playbackState.stream`'s `playing` transition triggers an immediate force-report on pause/stop, and a `mediaItem` change away from an episode force-reports its last known position. This avoids introducing a new polling primitive into the player stack — the existing stream-driven pattern (also used by `NowPlayingPersistence`) is already proven and testable via `StreamController`s without real `audio_service` platform channels.
3. **`played` is not derived from playback-completion detection in this milestone.** `EpisodeProgressController` always reports `played: false`; only `position_s` sync shipped. Completion detection (`AudioProcessingState.completed`) is deferred — out of scope for "throttled progress PUT (~every 15s + on pause/stop)," which is a position sync, not a played-state feature.
4. **The Now Playing / Mini Player screens were not retrofitted with episode-specific chrome** (no episode title/show/artwork treatment beyond what `MediaItem.title`/`artUri` already carry, no episode download-state button, no "Podcast" badge parallel to `isPreviewMediaItem`'s "Preview" badge). Playback itself works — `HeerrAudioHandler.playSong` is fully generic — but the rich UI a user would expect (skip-15s, playback speed, show name) is a follow-up. Flagged in `CHANGELOG.md`, not silently omitted.
5. **No on-device manual smoke.** No `adb` device was connected during this implementation session; PC5's own test gate (unit tests for the `MediaItem` builder + progress-reporter throttling) is green, but the "Smoke (on-device)" line in `ROADMAP.md` PC5 (subscribe → download → play offline with seek/resume; stream undownloaded; kill+reopen resume; progress reflected on the row) has **not** been run against a real device or backend.

**Why:** Building against the roadmap's literal `file://` wording would have required either a nonexistent on-device episode-download feature or silently falling back to always streaming (defeating "offline playback" for downloaded episodes). Re-deriving the correct behavior from the actual backend contract (`EpisodeItem.downloaded` + the Range-audio endpoint, both shipped at backend P5/P6) keeps the client honest to what the server can actually do — the same "adapt the plan to what shipped" precedent set repeatedly through backend P1–P6 and the earlier PC1–PC4 milestones (e.g. no separate subscription model, no `podcast_api.dart`, episodes excluded from the Sync Center's on-device-library cards).

**Alternatives considered:**
- **`Timer.periodic(15s)` reading `player.position` directly** — considered first, rejected in favor of the stream-driven `ScrobbleController` shape once that pattern was found: fewer moving parts, no new Timer lifecycle to manage/dispose, and directly reuses a stream (`positionStream`) already proven in production.
- **Retrofitting Now Playing/Mini Player with full episode chrome in the same commit** — rejected: real UI work with no device to visually verify against the source mockups' conventions (this repo's UI screens are consistently built against reference mockups and smoke-tested); deferred as a named follow-up rather than shipped unverified.
- **Deriving `played` from `AudioProcessingState.completed`** — rejected for this milestone: adds another state transition to reason about without a device to confirm `just_audio`'s completion signal fires reliably for both the Range-proxy and public-enclosure playback paths.

## 2026-07-20 — PR1: podcasts move into Library + Show Detail redesign (#53)

**Context:** Podcasts shipped functionally complete (PC1–PC5, `v5.0.0`–`v5.0.2`) but the UI was minimal and buried — the only entry point was Profile → "Podcasts" → a plain subscriptions grid → a plain channel list. A user-supplied design ("heerr — Podcast Flow | Design & Implementation Guide") calls for podcasts as a first-class Library citizen. PR1 is phase 1 of 3 (see the plan at the time: `~/.claude-personal/plans/luminous-singing-pinwheel.md`) — Library integration + a richer show-detail screen + richer episode rows, Android-only.

**Decision:**
1. **Music/Podcasts is a manually-driven `TabController`, not `DefaultTabController` + `TabBarView`.** `TabBarView(children: [...])` builds all children eagerly (it's backed by a `SliverChildListDelegate`, not lazy), so pairing the top-level content switch with a `TabBarView` would build the Podcasts subtree (and fire `podcastSubscriptionsProvider`'s network call) even while the user is idle on Music. Using an explicit `TabController` for visuals only, with the body swapped by a plain `_contentController.index == 0 ? ... : ...` ternary, keeps the `GradientTabIndicator` look while only ever building the selected content.
2. **`SubscriptionsScreen`'s grid was extracted into `PodcastShowsGrid`/`PodcastShowCard`** (new file `screens/podcasts/podcast_shows_grid.dart`), reused by both the standalone `/podcasts/subscriptions` route (still reachable from Profile, unchanged) and the new Library "Podcasts > Shows" tab. No redirect was added from the old route — it already works, so per `/CLAUDE.md` §3 project-wide practice of minimizing blast radius, the Library tab is simply the new *additional*, more-discoverable entry point rather than a replacement requiring a router migration.
3. **`ChannelScreen` was replaced outright by `PodcastShowDetailScreen`** at the same route (`/podcasts/channel/:id`) — hero art, Continue/Following action row, client-derived Continue Listening + Latest Episode mini-sections (computed from the already-loaded episode page, no new backend call), and Episodes/About tabs. The "Following" toggle caches the last-known `PodcastChannel` in State so the hero doesn't blank out the instant the user unsubscribes (at which point `podcastSubscriptionsProvider` no longer carries that channel).
4. **Episodes/Downloads sub-tabs under Library > Podcasts render `EmptyState` placeholders**, not fake data — the backend has no cross-subscription episode feed yet (that's Phase PA/PR3). The Sort control (Newest/Oldest/Unplayed) from the design was deliberately **not** shipped in any form — offering non-functional options would be worse than omitting the control.

**Why:** Reuses every existing provider (`podcastSubscriptionsProvider`, `podcastEpisodesNotifierProvider`, `podcastEpisodeDownloadDispatcherProvider`) and playback action (`playEpisode`) unchanged — this is presentation-layer work, not a new data layer. Deferring the two aggregate-feed tabs (rather than faking them) keeps the "Coming soon" state honest about what backend Phase PA will actually unlock.

**Alternatives considered:**
- **Redirect `/podcasts/subscriptions` into `Routes.library?content=podcasts`** — considered per the original plan wording ("keep it working... so nothing regresses"), rejected once it became clear leaving the route alone was strictly lower-risk (zero router/Profile test changes) while still satisfying "nothing regresses" literally.
- **`SegmentedButton<LibraryContent>`** for the top-level switch — considered, rejected in favor of a `TabBar` + `GradientTabIndicator` to visually match `_LibrarySegmentedTabs` and the rest of the app's tab styling, per the plan's explicit instruction.
- **Ship the Sort control disabled/stubbed** — rejected; per the plan, omitting it until PR3's backend `sort` param lands avoids shipping dead UI.

## 2026-07-20 — PR2: podcast player redesign (#53)

**Context:** Phase 2 of the podcast-flow redesign. Episodes already flowed through the player as a fourth `MediaItem` kind (`isEpisodeMediaItem`/`episodeIdFromMediaItem`, PC5), but the Now Playing screen rendered them with the music layout unchanged — waveform scrubber, prev/next-track transport, and a Lyrics/Add-to-playlist pill, none of which make sense for a single podcast episode.

**Decision:**
1. **`_Body` branches on `isEpisodeMediaItem(item)`** and delegates to a new sibling `_PodcastBody` rather than threading `if (episode) ...` branches through the existing music `_Body`. The two layouts share `_Header` and `_HeroArt` (with `song: null`, so no on-art download button — episodes have no on-device download affordance) but otherwise diverge completely: `_PodcastScrubber` (a stock `Slider`, not `WaveformSeekBar` — no amplitude data exists for podcast audio), `_PodcastTransport` (skip-back-30/play-pause/skip-forward-30, reusing the `_TapScale` + gradient-circle visual language of the music transport's center button), and `_PodcastActionPill` (Queue/Speed/Timer — Lyrics and Add to playlist dropped, neither applies to a single-episode "queue").
2. **`HeerrAudioHandler` gained `setSpeed` (an actual override of the `AudioHandler` base method — `flutter analyze` caught the missing `@override`), `skipBack30`, and `skipForward30`.** The skip methods compute the target offset directly (`position ± 30s`, clamped to `[0, duration]`) rather than relying on `SeekHandler`'s `fastForward`/`rewind` defaults — explicit and independently testable without depending on `audio_service`'s internal interval configuration.
3. **A speed picker bottom sheet (`_SpeedPickerSheet`)** mirrors `_SleepTimerSheet`'s visual language (same `ListTile` + checkmark pattern), offering 1.0×/1.25×/1.5×/1.75×/2.0×. Selecting a speed calls `setSpeed` and pops, same interaction shape as the sleep-timer presets.
4. **A tappable show-name link (`_PodcastShowLink`)** resolves the episode's channel title from `podcastSubscriptionsProvider` by the `channelId` extra `episode_to_media_item.dart` already stamps on every episode `MediaItem` — no new provider, reuses the same per-user subscription list every other podcast screen reads. `GoRouter.maybeOf` guards the tap so screens under test without a router ancestor don't crash.
5. **Chapters/Transcript/Notes/Bookmark and the show's "Related" tab are not implemented in any form** — no stubs, no placeholders — per the plan's scope decision (no backend/DB data backs any of it).

**Why:** A sibling widget rather than conditionals inside the existing `_Body` keeps both layouts independently readable and avoids scattering `if (episode)` checks across a component that was already large. Reusing `_TapScale`/`heerrGradient`/`_PillSlot`/`_PillDivider`/`_SleepCountdownChip` (all already `part of` the same library) keeps the podcast layout visually consistent with the music player without duplicating their implementations.

**Alternatives considered:**
- **`SeekHandler.fastForward`/`rewind` with a configured interval** — considered, rejected in favor of explicit `skipBack30`/`skipForward30` methods: clearer call sites (`ref.read(audioHandlerProvider).skipBack30()` reads better than a generic `fastForward()`), and avoids relying on `audio_service`'s default interval matching the design's fixed 30s (rather than being a configurable property that could silently drift).
- **A single `_Body` with branches instead of a `_PodcastBody` sibling** — rejected: the music layout already has ~110 lines of build logic (hero, title/artist/favourite row, scrubber, transport, pill, lyrics section); interleaving podcast branches throughout would make both paths harder to read than two focused widgets.

## 2026-07-20 — PR3: Home podcast sections + Library Episodes/Downloads + Show Detail sort (#53)

**Context:** Final phase of the podcast-flow redesign, unblocked by backend Phase PA (`v5.3.0`) — `GET /podcasts/episodes?filter=in_progress|latest|downloaded` and the per-channel `sort` param. Closes the two "Coming soon" placeholders PR1 shipped in Library's Podcasts section and adds the Music/Podcasts switch to Home.

**Decision:**
1. **One family provider, not three.** `podcastEpisodeFeedProvider(filter)` (`providers/podcasts/podcast_episode_feed.dart`) wraps `GET /podcasts/episodes?filter=`, family-keyed by the filter string. Five call sites (Home Continue Listening, Home Latest Episodes, Library Episodes, Library Downloads, plus a possible future one) share it rather than each getting a dedicated provider — the three filter values are already the backend's own vocabulary, so there's no benefit to renaming them per call site.
2. **`EpisodeWithChannel` gets an `toPodcastEpisode()` extension**, not a duplicate playback/download code path. `playEpisode` and the episode-download dispatcher (`podcastEpisodeDownloadDispatcherProvider`) both operate on `PodcastEpisode`; converting at the call site (`episode_with_channel.dart::EpisodeWithChannelPlayback`) means the cross-show feed rows on Home/Library reuse the exact same playback and download logic as the per-channel Show Detail episode rows, with zero duplication.
3. **Home's content switch is a manual `TabController` + conditional rendering, same pattern as Library's (`_LibraryContentSwitch`)** — not `DefaultTabController`/`TabBarView`, for the identical reason: `TabBarView(children: [...])` builds both children eagerly, which would fire the podcast feed providers' network calls the instant Home mounts, even while the user is idle on Music.
4. **Home's "Latest Episodes" is capped at 5 rows and non-scrollable** (a plain `Column`, not a nested `ListView`) — Home is a jumping-off point, not the full feed; the full, scrollable feed already lives at Library > Podcasts > Episodes. A nested `ListView` inside Home's outer `ListView` would also need unbounded-height handling for no real benefit.
5. **The Episodes-list sort control landed on the Show Detail screen (per-channel), not the Library Episodes tab (cross-show).** The plan's original phrasing ("wire the Phase 1 sort control... with the sort control now functional") assumed the two were the same screen; they aren't — PA2's `sort` param is per-channel only (the aggregate `/podcasts/episodes` endpoint has no `sort` param, only fixed per-`filter` orderings). Wiring the sort menu into `PodcastShowDetailScreen`'s Episodes tab (`PodcastEpisodesNotifier.setSort`) is the screen PA2 actually backs.
6. **`PodcastEpisodesNotifier.setSort` fetches directly and assigns `state`, not `ref.invalidateSelf()`.** `_sort` is an instance field, not part of the family key (`channelId` only) — `invalidateSelf()` disposes the current notifier and builds a fresh one, which would silently reset `_sort` to null on every sort change. Fetching directly (the same pattern `loadMore()` already uses) avoids the whole problem.

**Why:** Every new piece reuses an existing provider/action rather than inventing a parallel one — `playEpisode`, the download dispatcher, the `GradientTabIndicator` content-switch pattern, and the `EmptyState`/`SkeletonList`/`showApiError` conventions all carry over unchanged from PR1/PR2 and the earlier PC phases.

**Alternatives considered:**
- **Reuse `PodcastEpisodeFeedList` (Library's feed widget) inside Home's "Latest Episodes"** — rejected: that widget's internal `ListView.builder` is unbounded-height by design (meant to fill an `Expanded` tab body); nesting it inside Home's outer `ListView` hit exactly the "vertical viewport given unbounded height" error the SkeletonList loading-state fix below also had to solve. A capped, non-scrolling `Column` sidesteps it entirely for a section that's deliberately not the full list anyway.
...
## 2026-07-20 — fix: `NowPlayingPersistence.dispose()` race with CI test teardown (unrelated to #53)

**Context:** CI reported a flaky failure: `now_playing_persistence_test.dart`'s "debounced save fires once for a burst of trigger events" test occasionally failed with `now_playing_persistence: save failed: PathNotFoundException: Cannot rename file to '.../now_playing.json', ... No such file or directory`. Not a regression from the podcast work — the bug already existed in `NowPlayingPersistence.dispose()`.

**Decision:** `dispose()` now awaits `_pendingWrite` (the same write-serialization future `_enqueueWrite()`/`flush()` already chain onto) after canceling the debounce timer and trigger subscription, before returning.

**Why:** The debounce timer can fire and start `_writeSnapshot()` (write `.tmp` → rename) asynchronously; the old `dispose()` canceled the timer/subscription but returned immediately without waiting for a write already in flight from a timer that had *already* fired. Every test in this suite's `tearDown` calls `await persistence.dispose()` then deletes the test's temp directory — if the in-flight write's `tmp.rename()` hadn't completed yet, the delete could remove the `.tmp` file (or the whole directory) out from under the rename, throwing `PathNotFoundException`. This is a genuine race, not a test artifact: on a loaded/slow CI runner, the ~40ms margin between the test's 60ms fixed delay and the 20ms debounce was sometimes insufficient for the actual disk I/O to finish. Awaiting `_pendingWrite` in `dispose()` is safe unconditionally — canceling the timer and subscription first guarantees no *new* write can join the chain afterward, so the await can only ever wait on a write already in flight, never block indefinitely.

**Alternatives considered:**
- **Make the test wait deterministically instead of a fixed delay** (e.g. expose a "wait for pending write" hook) — rejected as the primary fix: even with a better test, the underlying `dispose()` race is a real production concern (e.g. a hot-restarted provider re-creating `NowPlayingPersistence` against the same file right after the old instance's `dispose()`) and should be fixed at the source. A deterministic regression test was still added (`_GatedStore`, a fake `NowPlayingStore` whose `save()` blocks on a `Completer` so the race is exercised without depending on real timers/IO) to prove `dispose()` genuinely waits — the fix wouldn't be provable/lock-in-place with only a timing-based test.
